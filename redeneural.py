...
class RedeNeural:
    ...
    def __init__(
        self,
        dados_entrada: np.ndarray,
        rotulos: np.ndarray,
        neuronios_ocultos: int = 10,
        neuronios_saida: int = 2,
    ):
        ...
        self.Pesos1 = np.random.randn(
            self.neuronios_camada_entrada, self.neuronios_camada_oculta
        ) / np.sqrt(self.neuronios_camada_entrada)
        self.Bias1 = np.zeros((1, self.neuronios_camada_oculta))
        self.Pesos2 = np.random.randn(
            self.neuronios_camada_oculta, self.neuronios_camada_saida
        ) / np.sqrt(self.neuronios_camada_oculta)
        self.Bias2 = np.zeros((1, self.neuronios_camada_saida))

        # Dicionário para fácil acesso aos parâmetros do modelo
        self.dicionario_modelo = {
            "Pesos1": self.Pesos1,
            "Bias1": self.Bias1,
            "Pesos2": self.Pesos2,
            "Bias2": self.Bias2,
    ...
    def propagacao_direta(self, dados_teste=None):
        ...
        saida_softmax = valores_exponenciais / np.sum(
            valores_exponenciais, axis=1, keepdims=True
        )
    ...
    def calcular_perda(self, saida_softmax):
        ...
        probabilidades_classes_corretas = saida_softmax[
            range(numero_amostras), self.rotulos
        ]

        # Evita o cálculo de log(0)
        probabilidades_classes_corretas = np.clip(
            probabilidades_classes_corretas, 1e-15, 1 - 1e-15
        )
    ...
    def retropropagacao(
        self, saida_softmax: np.ndarray, taxa_aprendizado: float = 0.01
    ) -> None:
        ...
        erro_camada_oculta = erro_camada_saida.dot(self.Pesos2.T) * (
    ...
    def treinar(self, epocas: int, taxa_aprendizado: float = 0.01):
        ...
        for epoca_atual in range(epocas):
            ...
            if (epoca_atual + 1) % max(1, epocas // 10) == 0 or epoca_atual == 0:
                ...
                print(
                    f"Época: [{epoca_atual + 1}/{epocas}] | Perda: {perda:.4f} | Acurácia: {acuracia:.4f}"
                )
